...
// TODO: rewrite the functions above to use normal ints rather than atomics

static inline u64 swap(RHeap* heap, u64 loc, u64 term) {
  u64 old = heap->buff[loc];
  heap->buff[loc] = term;
  return old;
}

static inline u64 get(RHeap* heap, u64 loc) {
  return heap->buff[loc];
}
...
static inline void alloc_redex(RHeap* heap, u64 neg, u64 pos) {
  u64 loc = heap->rbag_end;
  heap->rbag_end += 2;
  set(heap, RBAG + loc + 0, neg);
  set(heap, RBAG + loc + 1, pos);
}
...
int main() {
  ...
  u64* buff = calloc((1ULL << 33), sizeof(u64));
